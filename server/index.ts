...
async function startServer() {
  ...
  // API: Upload & Parse Excel Takeoff (.xlsx)
  app.post("/api/upload-excel", upload.single("file"), (req, res) => {
    try {
      ...
      if (!vesselData.systems) {
        vesselData.systems = { hvac: { schedule: [] }, electrical: { schedule: [] }, plumbing: { schedule: [] }, ancillary: { schedule: [] } };
      }
  ...
  const staticPath = process.env.NODE_ENV === "production"
    ...
    : path.resolve(__dirname, "..", "dist", "Here is the complete, corrected code for your `server/index.ts` file. It includes the required `multer` and `xlsx` imports at the top, and places the Excel upload route securely inside the `startServer()` function where `app` is defined.

### Replace `server/index.ts` with this complete code:
...
async function startServer() {
  ...
  app.post("/api/upload-excel", upload.single("file"), (req, res) => {
    try {
      ...
      if (!vesselData.systems) {
        vesselData.systems = { hvac: { description: "", flowSteps: [], specs: [], schedule: [] }, electrical: { description: "", flowSteps: [], specs: [], schedule: [] }, plumbing: { description: "", flowSteps: [], specs: [], schedule: [] }, ancillary: { description: "", flowSteps: [], specs: [], schedule: [] } };
      }
