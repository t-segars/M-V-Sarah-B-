    ...
    async function startServer() {
      ...
      const staticPath =
        process.env.NODE_ENV === "production"
          ...
          : path.resolve(__dirname, "..", "Ah, notice what happened in the build log! Some chat explanation text accidentally got pasted onto line 91 inside `server/index.ts`, causing an `Unterminated string literal` error and preventing `esbuild` from compiling `server/index.ts` into `dist/index.js`. That's why the script wasn't found.

Let's replace `server/index.ts` with this clean, pure TypeScript file without any stray text:

<Steps>
  <Step subtitle="Clean server/index.ts" title="Overwrite server/index.ts">
    Replace the entire contents of your `server/index.ts` file with this exact code:
